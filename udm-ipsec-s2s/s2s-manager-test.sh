...
wireguard_add_client() {
    ...
    wireguard_apply || { error "Client saved, but server restart failed."; pause; return; }
...
wireguard_remove_client() {
    ...
    printf '%-28s %s\n' "Client:" "${WG_CLIENT_NAME}"
    printf '%-28s %s\n' "VPN IP:" "${WG_CLIENT_IP}"
    echo; confirm_yes_no "Remove this client and restart WireGuard?" "N" || return
    rm -f "$(wireguard_client_state_file "${SELECTED_WG_CLIENT}")" "$(wireguard_client_export_file "${SELECTED_WG_CLIENT}")"
    wireguard_apply && ok "Client removed." || error "Client removed from state, but restart failed."
    pause
}
